...
- **사용 범위 전제**: 본 목업은 Daum(카카오 그룹) 사내에서 기능 기획·리뷰용으로만 사용. 외부 배포·일반 공개 용도 아님
- **헤드라인/제목**: 실제 m.daum.net 콘텐츠를 그대로 가져와 사실성을 높여도 무방. 단, 외부 공개 가능성이 있는 빌드에서는 가상 제목으로 마스킹 필요
- **이미지**: 다음 전략 중 선택
  - **실 콘텐츠 이미지 (사내용 권장)**: m.daum.net / news.daum.net 기사의 og:image를 추출해 `public/mock-images/` 하위에 저장 후 정적 서빙. URL 만료 영향을 받지 않고, 사내 도메인 이미지라 라이선스 이슈 없음
  - **일반 placeholder**: `src/lib/img.ts`의 `placeholderImg(seed, w, h, topic)` — loremflickr 기반, seed 해시로 결정적
  - **특정 인물이 필요한 섹션(연예·머니 탭) 외부 대체**: Wikimedia Commons CC 라이선스 이미지 URL을 mock 파일(`IMG` 상수)에 수동 큐레이션. 썸네일(`/thumb/`) 경로는 일부 리전에서 400 반환하므로 **원본 URL** 사용
  - 외부 이미지를 새로 가져올 땐 저작권/초상권 이슈 확인 필수. 정치인·사건 연루 인물은 피하기
