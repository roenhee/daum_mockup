import { Bookmark, ChevronDown, MapPin, MoreHorizontal, Play, Settings2, Share2, Sparkles, TrendingDown, TrendingUp, Users } from 'lucide-react';
...
const CHIP = 'inline-flex items-center px-2 py-[2px] rounded-chip bg-[#F0F0F0] text-[11px] text-gray-700';
...
function ActionRow({ primary = '원문 보기' }: { primary?: string }) {
  return (
    <div className="flex items-center gap-3 text-[12px] text-gray-500">
      <button type="button" className="font-medium text-gray-900">{primary}</button>
      <span className="w-px h-3 bg-gray-200" />
      <button type="button" aria-label="저장" className="flex items-center gap-1">
        <Bookmark size={14} />
        <span>저장</span>
      </button>
...
export function N1ArticleSingle({ data }: { data: ArticleSingleData }) {
  return (
    <article className="bg-white px-4 py-3.5">
      ...
      {data.contextTag ? (
        <p className="mt-2 text-[11px] text-gray-500 italic">{data.contextTag}</p>
      ) : null}
      <div className="mt-3">
        <ActionRow />
      </div>
...
export function N1ArticleMulti({ data }: { data: ArticleMultiData }) {
  return (
    <article className="bg-white px-4 py-3.5">
      ...
      <button
        type="button"
        className="mt-2 inline-flex items-center gap-1 text-[12px] text-gray-700"
      >
        다른 매체 {data.otherCount}개 보기
        <ChevronDown size={14} />
      </button>
      <div className="mt-3">
        <ActionRow />
...
export function N8Followup({ data }: { data: FollowupData }) {
  return (
    <article className="bg-white pl-4 pr-4 py-3.5 border-l-2 border-dashed border-[#4A4A4A]/40 ml-4 mr-4 rounded-r-md bg-gray-50/40">
...
export function N4KeywordRec({ data }: { data: KeywordRecData }) {
  ...
  return (
    <article className="bg-white px-4 py-3.5 border-l-[3px] border-[#4A4A4A]">
      <div className="flex items-center gap-1.5 text-[11px] text-gray-500">
        <Sparkles size={13} />
      ...
      <p className={cn('mt-2', FOOTER_AI)}>AI가 정리</p>
    </article>
  );
}
...
export function N14Trending({ data }: { data: TrendingData }) {
  return (
    <article className="bg-[#F5F5F5] px-4 py-3.5">
