...
interface SamePublisherSectionProps {
  publisherName: string;
  articles: NewsArticle[];
}

export function SamePublisherSection({
  publisherName,
  articles,
}: SamePublisherSectionProps) {
  return (
    <section className="bg-white">
      <SectionHeader title={`${publisherName}의 다른 기사`} />
          ...
          <li key={a.id}>
            <Link to={`/news/${a.id}`} className="flex gap-3 py-3">
              <div className="flex-1 min-w-0">
                ...
                <p className="mt-1 text-[11px] text-gray-500">{a.publishedAt}</p>
              </div>
              <img
                src={a.thumbnailUrl}
                alt=""
      ...
      <SectionMoreButton label={`${publisherName} 홈`} />
    </section>
  );
}
